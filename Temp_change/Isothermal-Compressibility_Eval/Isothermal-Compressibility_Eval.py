...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 04:54:18 2025
...
while i <= 10:
    temp += [i*2500]
    filename = f'{file_name}Research/MD/Temp_change/Al-melting/Outputs_NPTpressup/samp/samp_300K{temp[i-1]}bar.out'
    log_file = filename
...
c = polyFit(x,y,1)
quad = stdDev(c,x,y)
Iso = -1*(c[1]/np.mean(avg))
print('\nAverage Isothermal compressibility:',+Iso,'bar^-1')
...
c = polyFit(x,y,2)
quad = stdDev(c,x,y)
Iso = -1*(((300*c[2]*2)+c[1])/np.mean(avg))
print('\nAverage Isothermal compressibility:',+Iso,'bar^-1')
...
for i in range(len(temp)):
    print('|Pressure:',temp[i],'|Volume:',avg[i],'|Error:+-',error[i],'|')
...
func = (c[1]*x1)+c[0]
f = lambdify(x1, func, 'numpy')
xval = np.arange(2500, 26000, 2500)
func = f(xval)
total, residual = 0, 0
...
for i in range(len(temp)):
    iso += [-1*(c[1]/avg[i])]
    print('|Pressure:',temp[i],'|Isothermal compressibility:',iso[i],'|')
