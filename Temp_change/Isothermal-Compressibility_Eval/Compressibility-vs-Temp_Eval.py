...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 05:51:41 2025
...
while i <= 12:
    ...
    for idx, row in df.iterrows():
        steps += 1
        vol += row['vol']
        vol2 += (row['vol']*row['vol'])
        Elist += [row['vol']]
        E2list += [(row['vol']*row['vol'])]
    KbT2 = ((temp[i-2]**2)*8.617333262145*(10**(-5)))
    avg += [((vol2/steps)-((vol/steps)**2))/(KbT2*(vol/steps))]
...
plt.plot(temp, avg, label='Compressibility', marker='o', color='b')
plt.plot(xval, func, '--', label='Linear fit', color='r')
plt.plot(xval2, func2, label='Quadratic fit', color='g')
plt.xlabel('Temp (K)')
plt.ylabel('Compressibility (1/bar)')
plt.title('Compressibility vs. Temp')
plt.grid(True)
plt.legend()
plt.show()
...
for i in range(len(temp)):
    print('|Temp:',temp[i],'|Compressibility:',avg[i],'(1/bar)|Error:+-',error[i],'|')
