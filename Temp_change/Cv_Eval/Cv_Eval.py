...
while i <= 22:
    temp += [i*100]
    filename = f'{file_name}Research/MD/Temp_change/Al-melting/Outputs_NVTheating/samp/samp_{temp[i-1]}.out'
    log_file = filename
...
c = polyFit(x,y,1)
quad = stdDev(c,x,y)
print('\nConstant Volume Heat Capacity:',+c[1],'eV/K')
...
plt.plot(temp, avg, label='Internal Energy', marker='o', color='b')
...
for i in range(len(temp)):
    print('|Temp:',temp[i],'|Energy:',avg[i],'|Error:+-',error[i],'|')
...
func = (c[1]*x1)+c[0]
f = lambdify(x1, func, 'numpy')
xval = np.arange(100, 1900, 100)
func = f(xval)
total, residual = 0, 0
for i in range(len(func)):
    total += ((avg[i] - np.mean(avg[0:17])) ** 2)
