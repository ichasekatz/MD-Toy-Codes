...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 05:08:39 2025
...
while i <= 22:
    temp += [i*100]
    filename = f'{file_name}/MD/Temp_change/Al-melting/Outputs_NVTtoNPTheating/samp/samp_{temp[i-2]}.out'
    log_file = filename
...
c = polyFit(x,y,1)
quad = stdDev(c,x,y)
print('\nConstant Pressure Heat Capacity:',+c[1],'eV/K')
...
plt.plot(temp, avg, label='Enthalpy', marker='o', color='b')
plt.plot(xval, func, '--', label='Linear fit', color='r')
plt.plot(xval2, func2, label='Quadratic fit', color='g')
plt.xlabel('Temp (K)')
plt.ylabel('Enthalpy (eV/atom)')
plt.title('Enthalpy vs. Temp')
plt.grid(True)
plt.legend()
plt.show()
